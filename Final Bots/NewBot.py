...
def _protect():
    ...
    if len(my_icebergs) <= 2:
        return
    to_protect = all_icebergs if len(my_penguin_groups) >= len(
        enemy_penguin_groups) and have_more_icebergs() else my_icebergs
    ...
    for ice in my_icebergs:
        ...
        elif not ice.level < avg_level(enemy_icebergs) * UPGRADE_FACTOR and ice not in enemy_icebergs:
    ...
    if need_protect:
        for needIce in need_protect:
            ...
            for ice in can_help_ices:
                ...
                if Get_turns_till_arrival(ice, needIce) >= MAX_d // 3:
                    continue
...
def _attack():
    if bonus_iceberg.owner.equals(enemy_player):
    ...
    for dest in attack_dest:
        ...
        ]["acted"] not in ["Upgrade", "Bridge"] and not ice.level < avg_enemy_level * UPGRADE_FACTOR]
...
def ices_map(d):
    ...
    for ice in all_icebergs:
        for oth in all_icebergs:
