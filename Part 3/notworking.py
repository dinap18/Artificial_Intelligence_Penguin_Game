...
def do_turn(game):
    ...
    for my_iceberg in game.get_my_icebergs():
        ...
        if groups(my_iceberg, game.get_enemy_penguin_groups()) > my_iceberg.penguin_amount - destination_penguin_amount:
            continue
        elif not game.get_enemy_penguin_groups() and my_iceberg.can_upgrade() and game.turn > 10:
            my_iceberg.upgrade()
        # game.get_bonus_iceberg().owner == game.get_myself() and
        elif game.turn % 15 != 0 and (game.get_neutral_bonus_iceberg() and game.turn >= 57) or (
                groups(game.get_bonus_iceberg(),
                       game.get_enemy_penguin_groups()) >= game.get_bonus_iceberg().penguin_amount) and my_iceberg in bonusSorted[
                                                                                                                      0: len(
                                                                                                                          bonusSorted) // 2]:
            if groups(game.get_bonus_iceberg(), game.get_my_penguin_groups()) < 50:
                ...
                my_iceberg.level < 3 and my_iceberg.can_upgrade() and game.turn % 11 != 0) or (
                my_iceberg.level < 4 and my_iceberg.can_upgrade() and game.turn % 11 != 0):  # if it safe, upgrade.
            s += 1
            my_iceberg.upgrade()

        elif groups(mySorted[0], game.get_enemy_penguin_groups()) > mySorted[0].penguin_amount + groups(mySorted[0],
                                                                                                        game.get_my_penguin_groups()):
