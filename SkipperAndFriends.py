...
def do_turn(game):
    ...
    for my_iceberg in game.get_my_icebergs():
        ...
        if game.get_neutral_icebergs() and game.get_enemy_icebergs() and my_iceberg.get_turns_till_arrival(
            ...
            destination_penguin_amount = destination.penguin_amount // y - 2
        else:
            destination = enemySorted[0]
            if game.turn % 5 != 0:
                destination_penguin_amount = destination.penguin_amount // y - 2  # type: int
            else:
        ...
        if groups(my_iceberg, game.get_enemy_penguin_groups()) > my_iceberg.penguin_amount - destination_penguin_amount:
            s = True
        elif game.get_bonus_iceberg() and game.turn % 2 == 0 and groups(game.get_bonus_iceberg(),
                                                                        game.get_enemy_penguin_groups()) >= game.get_bonus_iceberg().penguin_amount:
